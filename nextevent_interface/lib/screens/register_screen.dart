...
// Manages stateful registration screen
  ...
  Widget buildInputField(TextEditingController controller, String label, IconData icon, {bool obscureText = false}) {
    return TextFormField(
      ...
      validator: (value) {
        ...
        if (label == "Email" && !value.contains(RegExp(r'^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$'))) {
          return "Please enter a valid email address";
        }
        if (label == "Password" && !value.contains(RegExp(r'^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[!@#\$&*~]).{8,}$'))) {
          return "Password must be at least 8 characters \n Must have one uppercase, lower letter & digit \n Must have one special character";
        }
